...
export function CardStack({
  ...
  const responsiveCardWidth = Math.min(cardWidth, windowWidth - 48);
            ...
            {items.map((item, i) => {
              ...
              return (
                <motion.div
                  key={item.id}
                  className={cn(
                    "absolute bottom-0 rounded-2xl overflow-visible",
...
function DefaultFanCard({ item }) {
  return (
    <div className="relative h-full w-full bg-white/[0.03] backdrop-blur-xl border border-white/[0.07] rounded-2xl p-8 flex flex-col justify-between shadow-2xl">
      {/* Top quotation mark decoration */}
      <div className="text-red-500/10 text-7xl font-serif leading-none absolute top-4 left-6 pointer-events-none select-none">“</div>
      
      {/* Review description */}
      <div className="relative z-10 mt-2 text-zinc-300 text-[0.95rem] leading-relaxed font-light italic">
        {item.description}
      </div>

      {/* User Info footer inside card */}
      <div className="relative z-10 flex items-center gap-4 border-t border-white/[0.05] pt-4 mt-4">
        {item.imageSrc && (
          <img
            src={item.imageSrc}
            alt={item.title}
        ...
        <div className="flex flex-col">
          ...
          {item.tag && (
            <span className="text-red-500 text-[10px] font-bold tracking-wider uppercase mt-0.5">
              {item.tag}
            </span>
          )}
        </div>
